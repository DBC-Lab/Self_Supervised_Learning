...
protopath=''
modelpath=''
mynet = caffe.Net(protopath+'deploy.prototxt',modelpath+'SegM_18.caffemodel',caffe.TEST)
...
def cropCubic(matT1,matT2,fileID,d,step,rate):
    eps=1e-5
    ...
    for i in range(aa,bb,step[0]):
        ...
        for j in range(cc,dd,step[1]):
            for k in range(ee,ff,step[2]):
                volMR[0,:,:,:] = matT1[i:i+d[0],j:j+d[1],k:k+d[2]]  #patch 
...
def main():
    ...
    for mli in range(len(monthList)):
        ...
        for idn in range(len(ids)):
            ...
            f_Img2 = os.path.join(data_path,'%s-T2.hdr'%subject_name);
            img2_Org = sitk.ReadImage(f_Img2)
            imgT2=sitk.GetArrayFromImage(img2_Org)

            fileID='%02d'%idn
            rate=1
            imgT1=np.transpose(imgT1,(0,2,1))
            imgT2=np.transpose(imgT2,(0,2,1))
            matOut,PrOut0,PrOut1,PrOut2,PrOut3=cropCubic(imgT1,imgT2,fileID,dFA,step,rate)
